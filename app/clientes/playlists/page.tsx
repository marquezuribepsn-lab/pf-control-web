...
const mkId = () => `pl-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;

export default function ClientesPlaylistsPage() {
  const { alumnos } = useAlumnos();
  const [assignments, setAssignments] = useSharedState<PlaylistAssignment[]>([], {
    key: STORAGE_KEY,
  ...
  return (
    <main className="mx-auto max-w-7xl p-6 text-slate-100">
      <div className="mb-6 rounded-2xl border border-fuchsia-400/30 bg-slate-900/70 p-6">
        <h1 className="text-3xl font-black">Playlists para clientes</h1>
        <p className="mt-2 text-sm text-slate-300">
          Asigna playlists de Spotify por alumno con objetivo, dia de uso y cancion recomendada.
        </p>
      </div>
      ...
      <section className="mt-4 rounded-2xl border border-white/15 bg-slate-900/70 p-5">
        ...
        <div className="mt-3 space-y-2">
          {sortedAssignments.length === 0 ? (
            <p className="text-sm text-slate-400">No hay playlists asignadas.</p>
          ) : null}
