...
$remoteBuild = "set -e; cd $RemoteDir; if [ -f ./.db.env ]; then set -a; . ./.db.env; set +a; fi; npm ci --legacy-peer-deps --no-audit --no-fund --loglevel=error; npm run db:migrate:deploy; npm run db:generate; npm run smoke:runtime:db; npm run guard:interaction-lock; npm run guard:mobile-scroll-lock; npm run guard:shell-profile-lock; npm run build; (pm2 describe pf-control-web > /dev/null 2>&1 && pm2 restart pf-control-web --update-env || pm2 start ecosystem.config.cjs --env production); npm run whatsapp:automation:cron:setup; npm run smoke:login:guard; npm run smoke:alumnos:rutina:back; npm run smoke:alumnos:nutricion; npm run smoke:dock:no-reload; npm run smoke:sidebar:order; npm run smoke:mail:guard; $mailIntensiveStep (npm run smoke:admin:usuarios:redirect || echo '[guardrail] smoke:admin:usuarios:redirect fallo (revisa SMOKE_MAIN_PASSWORD). Continuando para evitar magic-link automatico.'); $uxGuardrailStep pm2 save"
